  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex h-9 w-9 items-center justify-center rounded-lg text-black/55 transition hover:bg-black/5 hover:text-black dark:text-white/60 dark:hover:bg-white/5 dark:hover:text-white"
      >
        ...
        <>
          ...
          <div className="absolute right-0 top-full z-50 mt-2 w-56 overflow-hidden rounded-2xl border border-black/[0.08] bg-white p-2 shadow-xl dark:border-white/[0.1] dark:bg-zinc-950">
            ...
            <button
              type="button"
              onClick={toggleTheme}
              className="flex w-full items-center justify-between rounded-xl px-2.5 py-2 text-sm transition hover:bg-black/5 dark:hover:bg-white/5"
            >
            ...
            {themes.map((item) => (
              <button
                ...
                className="flex w-full items-center justify-between rounded-xl px-2.5 py-2 text-sm transition hover:bg-black/5 dark:hover:bg-white/5"
              >
                <span className="flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full bg-primary-500" />{item.label}</span>
                {colorTheme === item.id && <Check className="h-4 w-4 text-primary-500" />}
              </button>
            ))}
          </div>
        </>
