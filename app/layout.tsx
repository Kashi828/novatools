...
export const metadata: Metadata = { metadataBase: new URL(SITE_URL), title: { default: 'NovaTools — Free Online Tools That Save You Time', template: '%s | NovaTools' }, description: 'Hundreds of free online tools for text, images, PDFs, developers, calculators, and more. No installation required — most tools need no account at all.', keywords: ['online tools', 'free tools', 'calculator', 'converter', 'developer tools', 'pdf tools'], openGraph: { type: 'website', url: SITE_URL, title: 'NovaTools — Free Online Tools That Save You Time', description: 'Hundreds of free online tools. No installation required for most — just open and use.', siteName: 'NovaTools' }, twitter: { card: 'summary_large_image', title: 'NovaTools — Free Online Tools That Save You Time', description: 'Hundreds of free online tools. No installation required for most — just open and use.' }, robots: { index: true, follow: true }, manifest: '/manifest.json', icons: { icon: '/icon.svg', apple: '/icon.svg' } };

export default async function RootLayout({ children }: { children: React.ReactNode }) {
  const maintenance = await getMaintenanceSettings();
  const maintenanceActive = isMaintenanceActive(maintenance);
  const admin = maintenanceActive ? await isAdminUser() : false;
  const fontVars = `${playfair.variable} ${spaceGrotesk.variable} ${merriweather.variable} ${poppins.variable} ${inter.variable}`;
  return <ClerkProvider signInUrl="/sign-in" signUpUrl="/sign-up" appearance={{ variables: { colorPrimary: '#C9A961' } }}><html lang="en" className={fontVars} suppressHydrationWarning><body className="flex min-h-screen flex-col bg-noise"><script dangerouslySetInnerHTML={{ __html: appearanceBootstrap }} /><ThemeProvider><ThemeFavicon /><ToastProvider>{maintenanceActive && !admin ? <MaintenancePage settings={maintenance} /> : <><SiteAnnouncement /><PromoBanner /><Navbar /><CommandPaletteLoader /><main className="flex-1"><PageTransition>{children}</PageTransition></main><Footer /></>}</ToastProvider></ThemeProvider></body></html></ClerkProvider>;
}
