...
export default async function HomePage() {
  ...
  return <HomeMotion>
    <section className="relative overflow-hidden border-b border-black/[0.06] dark:border-white/[0.08]">
      ...
      <div className="mx-auto max-w-7xl px-4 pb-20 pt-16 sm:px-6 sm:pt-24 lg:px-8 lg:pb-28">
        <div className="mx-auto max-w-4xl text-center">
          <div className="mx-auto mb-6 inline-flex items-center gap-2 rounded-full border border-secondary-500/15 bg-white/[0.03] px-3.5 py-1.5 text-xs font-semibold text-secondary-400"><WandSparkles className="h-3.5 w-3.5" />A better toolbox for the web</div>
          <h1 className="text-balance text-5xl font-bold tracking-[-0.04em] sm:text-6xl lg:text-7xl">Everything useful,<span className="block text-secondary-300 dark:text-secondary-300">in one place.</span></h1>
    ...
    <section className="border-y border-black/[0.06] bg-black/[0.02] dark:border-white/[0.08] dark:bg-white/[0.02]"><div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8"><div className="mb-8 flex items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-secondary-400">Explore</p><h2 className="mt-2 text-3xl font-bold tracking-tight">Browse by category</h2></div><Link href="/categories" className="text-sm font-semibold text-secondary-400 hover:underline">View all</Link></div><div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">{categories.map((cat) => {const count = tools.filter((tool) => tool.category === cat.slug && !hidden.has(tool.slug)).length;return <Link key={cat.slug} href={`/categories/${cat.slug}`} className="group rounded-2xl border border-black/[0.07] bg-white/[0.025] p-5 backdrop-blur transition duration-200 hover:-translate-y-0.5 hover:border-secondary-500/20 hover:bg-white/[0.04] dark:border-white/[0.08]"><span className="mb-5 flex h-10 w-10 items-center justify-center rounded-xl bg-white/[0.05] text-secondary-400 transition group-hover:scale-105"><cat.icon className="h-5 w-5" /></span><span className="block text-sm font-semibold">{cat.name}</span><span className="mt-1 block text-xs text-black/45 dark:text-white/45">{count} tools</span></Link>;})}</div></div></section>
    <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8"><div className="grid gap-4 sm:grid-cols-3">{benefits.map(({ icon: Icon, title, text }) => <div key={title} className="rounded-2xl border border-black/[0.07] bg-white/[0.025] p-6 dark:border-white/[0.08]"><Icon className="mb-5 h-5 w-5 text-secondary-400" /><h3 className="font-semibold">{title}</h3><p className="mt-2 text-sm leading-6 text-black/55 dark:text-white/55">{text}</p></div>)}</div></section>
  </HomeMotion>;
}

function ToolSection({ title, tools: items, link }: { title: string; tools: typeof tools; link?: string }) { if (!items.length) return null; return <section className="py-8 first:pt-0"><div className="mb-5 flex items-center justify-between"><h2 className="text-2xl font-bold tracking-tight">{title}</h2>{link && <Link href={link} className="inline-flex items-center gap-1 text-sm font-semibold text-secondary-400 hover:underline">View all <ArrowRight className="h-3.5 w-3.5" /></Link>}</div><div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">{items.map((tool) => <Link key={tool.slug} href={`/tools/${tool.slug}`} className="group rounded-2xl border border-black/[0.07] bg-white/[0.025] p-5 shadow-sm transition duration-200 hover:-translate-y-1 hover:border-secondary-500/20 hover:bg-white/[0.04] hover:shadow-xl hover:shadow-black/10 dark:border-white/[0.08]"><div className="mb-5 flex h-11 w-11 items-center justify-center rounded-xl bg-white/[0.05] text-secondary-400"><span className="contents">{tool.icon && <tool.icon className="!h-5 !w-5 !text-secondary-400" />}</span></div><h3 className="font-semibold">{tool.name}</h3><p className="mt-1.5 line-clamp-2 text-sm leading-5 text-black/50 dark:text-white/50">{tool.description}</p><span className="mt-4 inline-flex items-center gap-1 text-xs font-semibold text-secondary-400 opacity-0 transition group-hover:opacity-100">Open <ArrowRight className="h-3 w-3" /></span></Link>)}</div></section>; }
