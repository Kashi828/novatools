...
function readStoredCustomColors(): CustomColors {
  ...
  try {
    ...
    if (typeof parsed.primary === 'string' && typeof parsed.secondary === 'string' && typeof parsed.accent === 'string') return { primary: parsed.primary, secondary: parsed.secondary, accent: parsed.accent };
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const storedTheme = readStored<Theme>('novatools-theme', html.classList.contains('dark') ? 'dark' : 'light');
    const storedColor = readStored<ColorTheme>('novatools-color-theme', (html.getAttribute('data-theme') as ColorTheme) || 'gold');
    ...
    setTheme(storedTheme); setColorThemeState(storedColor); setFontPairingState(storedFont); setRadiusStyleState(storedRadius); setUiScaleState(storedScale); setMotionPreferenceState(storedMotion); setCustomColorsState(storedCustom);
    html.classList.toggle('dark', storedTheme === 'dark');
    html.setAttribute('data-theme', storedColor === 'custom' ? 'gold' : storedColor);
    html.setAttribute('data-font', storedFont); html.setAttribute('data-radius', storedRadius); html.setAttribute('data-ui-scale', storedScale); html.setAttribute('data-motion', storedMotion);
    if (storedColor === 'custom') applyCustomColors(storedCustom); else clearCustomColors();
    writeCookie('novatools-theme', storedTheme); writeCookie('novatools-color-theme', storedColor); writeCookie('novatools-custom-colors', JSON.stringify(storedCustom));
    setReady(true);
  }, []);
