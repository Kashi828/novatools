...
const SECTION =
  'mt-3 border-t border-black/10 px-2 pt-3 text-xs font-medium uppercase tracking-wide text-black/40 dark:border-white/10 dark:text-white/40';
...
export function ThemePicker() {
  ...
  // Keeps a half-typed hex visible while the user edits it, without pushing an
  // invalid value into the live theme.
  ...
  const panel = (
    <div
      className="fixed inset-0 z-[100] flex items-start justify-center bg-black/45 p-4 pt-20 backdrop-blur-sm"
      onMouseDown={(event) => {
    ...
    >
      <div
        ...
        className="max-h-[calc(100vh-6rem)] w-full max-w-sm overflow-y-auto rounded-xl2 border border-black/10 bg-white p-2 shadow-2xl outline-none dark:border-white/10 dark:bg-[#111113]"
      >
          ...
          <div className="mt-1 space-y-2 border-t border-black/10 px-2 pt-3 dark:border-white/10">
            ...
            <div className="flex items-center justify-between gap-2 pt-1">
              <p className="text-[11px] text-black/40 dark:text-white/40">
                Pick 3 base colors — every shade is generated from them.
              </p>
              <button
                type="button"
                onClick={() => {
        ...
        <div className="flex gap-1.5 px-2 py-1.5">
          {RADII.map((radius) => (
            <button
              key={radius.key}
              type="button"
              onClick={() => setRadiusStyle(radius.key)}
              className={segment(radiusStyle === radius.key)}
            >
              {radius.label}
            </button>
          ))}
        ...
        <div className="mx-2 mb-2 mt-1 rounded-xl2 border border-black/10 p-3 dark:border-white/10">
          <p className="text-[11px] leading-snug text-black/45 dark:text-white/45">
            Live preview — corners scale together so labels never crowd the edge.
          </p>
