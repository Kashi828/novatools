...
function applyCustomColors(colors: CustomColors) {
  ...
  root.style.setProperty('--color-grad-1', primary[400]); root.style.setProperty('--color-grad-2', primary[500]); root.style.setProperty('--color-grad-3', secondary[500]); root.style.setProperty('--color-glow', primary[500]);
  root.style.setProperty('--logo-grad-1', colors.primary); root.style.setProperty('--logo-grad-2', colors.primary); root.style.setProperty('--logo-grad-3', colors.secondary);
}
function clearCustomColors() { const root = document.documentElement; PALETTE_VARS.forEach((prop) => root.style.removeProperty(prop)); }
...
function readStoredCustomColors(): CustomColors {
  ...
  const value = window.localStorage.getItem('novatools-custom-colors'); if (!value) return DEFAULT_CUSTOM_COLORS;
  try { const parsed = JSON.parse(value) as Partial<CustomColors>; if (typeof parsed.primary === 'string' && typeof parsed.secondary === 'string' && typeof parsed.accent === 'string') return { primary: parsed.primary, secondary: parsed.secondary, accent: parsed.accent }; } catch { /* ignore malformed local storage */ }
  return DEFAULT_CUSTOM_COLORS;
}
function parseColor(value: string): [number, number, number] | null {
  const hex = value.trim().match(/^#([0-9a-f]{6})$/i); if (hex) return [parseInt(hex[1].slice(0, 2), 16), parseInt(hex[1].slice(2, 4), 16), parseInt(hex[1].slice(4, 6), 16)];
  const numbers = value.match(/\d+(?:\.\d+)?/g); if (numbers && numbers.length >= 3) return [Number(numbers[0]), Number(numbers[1]), Number(numbers[2])];
  return null;
}
...
function capturePalette() {
  const styles = getComputedStyle(document.documentElement); return Object.fromEntries(PALETTE_VARS.map((name) => [name, styles.getPropertyValue(name).trim()])) as Record<(typeof PALETTE_VARS)[number], string>;
}
function animatePalette(applyTarget: () => void, keepInline: boolean) {
  ...
  const tick = (now: number) => {
    ...
    PALETTE_VARS.forEach((name) => {
      const a = parseColor(from[name]); const b = parseColor(to[name]);
      if (!a || !b) return;
      root.style.setProperty(name, colorString([a[0] + (b[0] - a[0]) * eased, a[1] + (b[1] - a[1]) * eased, a[2] + (b[2] - a[2]) * eased]));
    });
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => { if (!ready) return; localStorage.setItem('novatools-color-theme', colorTheme); writeCookie('novatools-color-theme', colorTheme); localStorage.setItem('novatools-custom-colors', JSON.stringify(customColors)); writeCookie('novatools-custom-colors', JSON.stringify(customColors)); }, [colorTheme, customColors, ready]);
  function setColorTheme(t: ColorTheme) {
    localStorage.setItem('novatools-color-theme', t); writeCookie('novatools-color-theme', t);
    animatePalette(() => { document.documentElement.setAttribute('data-theme', t === 'custom' ? 'gold' : t); if (t === 'custom') applyCustomColors(customColors); else clearCustomColors(); }, t === 'custom');
    setColorThemeState(t);
  }
  function setCustomColors(c: CustomColors) {
    localStorage.setItem('novatools-custom-colors', JSON.stringify(c)); writeCookie('novatools-custom-colors', JSON.stringify(c));
    if (colorTheme === 'custom') animatePalette(() => applyCustomColors(c), true);
